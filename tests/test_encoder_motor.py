...
def main():
    ...
    for turn, actual in turns:
        ...
        if actual - margin_of_error < encoder_angle < actual + margin_of_error:
            print("Observed value " + str(encoder_angle) + " tracks to " + str(actual) + ".")
        else:
            print("Encoder outside of tolerance! " + str(actual) + " vs " + str(encoder_angle))
        time.sleep(1)
        motor.reset()
